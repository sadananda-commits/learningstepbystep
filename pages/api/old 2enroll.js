...
export default async function handler(req, res) {
  ...
  if (!SHEETDB_URL) {
    return res.status(500).json({ message: 'SHEETDB_URL is missing from .env.local' });
  }
  ...
  try {
    // SheetDB: POST to specific sheet using /sheet/{name} path
    ...
    // SheetDB: POST to Accounts sheet
