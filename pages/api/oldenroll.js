...
export default async function handler(req, res) {
  ...
  if (!SHEETDB_URL) {
    return res.status(500).json({ message: 'Server not configured: SHEETDB_URL missing in .env.local' });
  }
  ...
  try {
    ...
    if (!enrollRes.ok) {
      ...
      console.error('SheetDB Enrollments error:', err);
    ...
    if (!acctRes.ok) {
      ...
      console.error('SheetDB Accounts error:', err);
      return res.status(500).json({ message: 'Enrollment saved but account creation failed.' });
    }
    ...
    return res.status(500).json({ message: 'Failed to reach database. Please try again.' });
  }
}
